  ...
  io.on('connection', (socket) => {
    ...
    socket.on('video-call-invite', async (data) => {
      try {
        ...
        await sendCallMessageIfNotExists({
...
const sendCallMessageIfNotExists = async ({ conversationId, senderPhone, receiverPhone, status, duration, type, callId }) => {
  // Kiểm tra đã có message call với callId và status này chưa
  const params = {
    TableName: process.env.MESSAGE_TABLE,
    IndexName: 'conversationIndex',
    KeyConditionExpression: 'conversationId = :conversationId',
  ...
  const result = await dynamoDB.query(params).promise();
  const existed = result.Items && result.Items.some(m => m.callId === callId && m.callStatus === status);
  if (!existed) {
    await sendCallMessage({ conversationId, senderPhone, receiverPhone, status, duration, type, callId });
  }
};
